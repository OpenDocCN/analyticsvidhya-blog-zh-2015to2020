...
术语**可接受性**是指被认为形成集合的单个模型在执行任务时应该是彼此可接受的。用简单的统计学术语来说，单个模型做出正确预测的概率应该比任何随机模型都要好。同样的陈述可以被量化为单个模型正确预测的概率应该大于 0.5。
...
其中 C 表示相同模型类的不同模型，具有在训练数据的不同子集上训练的相同超参数组合。
...
![](img/96b1a82b60fd8a36c988480e36ba4cdf.png)
