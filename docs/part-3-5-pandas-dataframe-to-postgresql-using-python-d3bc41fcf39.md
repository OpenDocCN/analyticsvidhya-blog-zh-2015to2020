# 第 3.5 部分！！使用 Python 将数据帧转换为 PostgreSQL

> 原文：<https://medium.com/analytics-vidhya/part-3-5-pandas-dataframe-to-postgresql-using-python-d3bc41fcf39?source=collection_archive---------0----------------------->

## 使用 copy_from()将批量数据插入 PostgreSQL 数据库

![](img/a60b8c49a1e475342ae112d050048ae1.png)
...
本文的主要目标是逐步学习 ***copy_from()*** 方法的工作代码。
...
def connect(conn_params_dic):**
    ...
    try:
        ...
        **# passing exception to function**
        ...
        conn = None
    return conn***# Define function using copy_from_dataFile to insert the dataframe.***
...
# irisCopyFromFile table [Note: first create irisCopyFromFile
# table into the irisdb(***[***Part 2***](/analytics-vidhya/pandas-dataframe-to-postgresql-using-python-part-2-3ddb41f473bd)***)].***
copy_from_dataFile(conn, irisData, 'irisCopyFromFile')
...
```
***# Prepare sql query***
sql = "SELECT * FROM irisCopyFromFile" 
cursor = conn.cursor()
***# Execute cursor***
cursor.execute(sql)
...
**结论:**我们的**部分 3.5** 到此结束。在本教程中，我们学习了如何使用 ***copy_from()*** 方法将批量数据插入 PostgreSQL 数据库。

本文的所有代码都可以作为 GitHub *上的 [***Jupyter 笔记本获得。***](https://github.com/Muhd-Shahid/Learn-Python-Data-Access/tree/main/PostgreSQL)*

> **下一个** [**第 3.6 部分**](https://shahid-dhn.medium.com/part-3-6-pandas-dataframe-to-postgresql-using-python-ec80cb33ca4a) **:使用 Python** 使用 StringIO 方法使用 *copy_from()将批量数据插入 PostgreSQL 数据库*
> 
> ***之前的学习:***
> 
