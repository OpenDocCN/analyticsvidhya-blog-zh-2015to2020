...
> 原文：<https://medium.com/analytics-vidhya/chatbot-human-handoff-seamless-human-takeover-in-a-hybrid-solution-c6d876b88013?source=collection_archive---------1----------------------->

![](img/f0bc3c2e77fecaac6b10ceaf3f966c47.png)
...
因此，在一个机器人+人类的混合解决方案中，聊天机器人是你的第一道防线，当这条防线被攻破时，人类代理人就会出现。现在，对于像我们这样的混合解决方案，确保用户体验到从机器人到人工代理的平稳过渡变得至关重要。
...
所以在这种情况下，对话应该交给人工代理。因此，机器人必须足够聪明，知道自己能力的极限，并提出替代方案。这是大多数聊天机器人失败的地方。你经常会发现机器人在问同样的问题，即使在多次无法理解意图之后。
...
呈现该选项的最佳实践是通过一个可操作的按钮，用户可以简单地点击它并连接到一个人工代理。

另一种方式是，机器人询问用户是否愿意连接到人类代理，然后根据他的响应来连接。

让用户了解正在发生的转变也很重要。这将设置一个预期，即在移交过程中，票证是未分配的，在此期间发送的问题只有在分配代理后才会得到回答。**鸣谢:**
...
我们将查询的性质作为一个因素，即基于平台类型(Android、iOS、web 等..)我们将其分配给合适的支持工程师。**上下文分配:**这种分配涉及复杂的逻辑，根据上下文因素分配代理。这些因素可以是任何东西，例如用户的地理或语言。在
...
有两种方法可以在通信中实现机器人到人的切换。您可以从代码中实现它，也可以在从 UI 本身创建 bot 时启用它。当你在通信中创建你的机器人时，最后一步你会被要求启用机器人与人类的交接。

![](img/72447805ebcf6a7effa0cb06675a1969.png)

第二种方法是添加一小段代码。下面的代码示例显示了与 Kommunicate 集成的 Dialogflow 机器人的聊天机器人人工切换方法
...
*原载于 2019 年 2 月 11 日*[*https://www . komunicate . io*](https://www.kommunicate.io/blog/chatbot-human-handoff/)*。*